...
BOOL DownFileFunc(SOCKET m_Sock){
	...
	if (SendData(m_Sock, _T("请输入目标文件的URL："), _tcslen(_T("请输入目标文件的URL："))) > 0 && 
		GetCmdString(m_Sock, szDstUrl)) {
		if (SendData(m_Sock, _T("请输入目标文件的保存路径："), _tcslen(_T("请输入目标文件的保存路径："))) > 0 && 
